...
//  float temp = (map(eyebrow, 6, 10, 6, 10)); //map eyebrow height low to high to two values
...
//println("tempint = " +tempint);
 // String pos = str(tempint); //make this a string "make this a string called "pos"
  ...
  if (eyebrow > 7 && eyebrow < 8.5) {  //anywhere from 7.001 to 8.49
    client.publish("/eyebrows", "neutral"); //neutral
    background(250, 250, 0);
    rect(170, 25, 25, 300);
    } else if (eyebrow < 7) { // below 7
      client.publish("/eyebrows", "frown"); //frown
      background(0, 250, 0);
      rect(150, 25, 25, 300);
      rect(190, 25, 25, 300);
      } else if (eyebrow > 8.5) { //above 8.5
        client.publish("/eyebrows", "raise"); //raise
        background(0, 0, 250);
        rect(130, 25, 25, 300);
        rect(170, 25, 25, 300);
        rect(210, 25, 25, 300);
